...
const getErrorMessage = (error) => {
  ...
  const mappings = [
    { keywords: ["doctor", "Doctor reference is required"], message: "Please select a doctor." },
    { keywords: ["patient", "Patient reference is required"], message: "Please select a patient." },
    { keywords: ["Appointment validation failed", "appointment validation"], message: "Please complete all appointment details." },
    { keywords: ["ValidationError", "Validation", "validation failed"], message: "Please check the information you entered." },
    { keywords: ["duplicate", "already exists", "E11000"], message: "A record with these details already exists." },
    { keywords: ["PAN", "pan"], message: "Please enter a valid PAN number." },
    { keywords: ["Email", "email"], message: "Please enter a valid email address." },
    { keywords: ["mobile", "phone", "Mobile"], message: "Please enter a valid 10-digit mobile number." },
    { keywords: ["No patients found", "No patient found"], message: "No patients are available." },
    { keywords: ["No Appoinments found", "No appointments found"], message: "No appointments are scheduled." },
    { keywords: ["Access Denied", "authorization", "unauthorized"], message: "You do not have permission to perform this action." },
    { keywords: ["token", "invalid token"], message: "Your session has expired. Please login again." },
    { keywords: ["time slot", "already booked"], message: "This time slot is already booked. Please select another time." },
    { keywords: ["discharge", "pending dues"], message: "Please clear all pending dues first." }
  ];

  for (const item of mappings) {
    if (item.keywords.some((k) => msg.toLowerCase().includes(k.toLowerCase()))) {
      return item.message;
    }
  }
...
function getDoctorName(appointment, doctorsList) {
  ...
  if (appointment.assignedDoctorId) {
    const doc = doctorsList?.find(d => d._id === appointment.assignedDoctorId);
    if (doc) return doc.fullname;
  }
  if (appointment.assignedDoctorName) {
    const doc = doctorsList?.find(d => d._id === appointment.assignedDoctorName || d.fullname === appointment.assignedDoctorName);
...
export default function ManagerDashboard({ role }) {
  ...
  const filtered = appointments.filter((a) => {
    ...
    const doctorName = getDoctorName(a, doctors).toLowerCase();
  ...
  const selectedDoctorInfo = doctors.find(d => 
    d._id === newAppointmentData.assignedDoctorId
  );
  ...
  const handleDischargeConfirm = async () => {
    try {
      ...
      await api.put(`/patientapi/discharge/${userId}`, {
        patientId: dischargeModal.req.patient?._id || dischargeModal.req.patientId || dischargeModal.req.patient,
                      ...
                      {loading ? (
                        ...
                        filtered.map((a) => (
                          <tr key={a._id || a.id} className="border-b border-gray-50 hover:bg-blue-50/50 transition-colors">
                            ...
                            <td className="px-5 py-4 text-gray-600">{getDoctorName(a, doctors)}</td>
                            <td className="px-5 py-4 text-gray-500">{getSpecialization(a, doctors)}</td>
                    ...
                    {wizardStep < 4 && (
                      <div className="flex items-center gap-4 mb-6 text-sm font-medium">
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center shadow-sm ${wizardStep >= 1 ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                            {wizardStep > 1 ? <Check className="w-3.5 h-3.5" strokeWidth={3} /> : 1}
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center shadow-sm ${wizardStep >= 2 ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                            {wizardStep > 2 ? <Check className="w-3.5 h-3.5" strokeWidth={3} /> : 2}
                          </div>
                      ...
                      <div>
                        ...
                        {patientMode === 'new' ? (
                          <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <label className="space-y-1">
                              <span className="text-sm font-medium text-gray-700">Mobile Phone (10 digits)</span>
                              <input 
                                type="tel" 
                                maxLength={10} 
                                value={newAppointmentData.mobilePhone} 
                                onChange={(e) => setNewAppointmentData({ ...newAppointmentData, mobilePhone: e.target.value.replace(/\D/g, '') })} 
                                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" 
                              />
                            </label>
                            <label className="space-y-1">
                      ...
                      <div className="space-y-5">
                        <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm flex gap-4 shadow-sm">
                          <div>
                            ...
                            <span className="font-semibold text-gray-900">{newAppointmentData.name || 'Unknown'}</span>
                          </div>
                          <div className="w-px h-8 bg-blue-200"></div>
                          <div>
                            <span className="text-gray-500 text-xs block">PID</span>
                            <span className="font-semibold text-gray-900">{newAppointmentData.pid}</span>
                          </div>
                        </div>
                          ...
                          {showTimeDropdown && (
                            <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto mt-1 divide-y divide-gray-100">
                              {availableSlots.length > 0 ? (
                                availableSlots
                                  .filter(s => s.val24.replace(/^0/, '').startsWith(timeQuery.replace(/^0/, '')))
                                  .length > 0 ? (
                                  availableSlots
                                    .filter(s => s.val24.replace(/^0/, '').startsWith(timeQuery.replace(/^0/, '')))
                                    .map(slot => (
                                      <li 
                                        ...
                                        onClick={() => { 
                                          ...
                                          setNewAppointmentData({...newAppointmentData, appointmentTime: slot.val12});
                                          setShowTimeDropdown(false);
                                        }}
                    ...
                    {wizardStep === 3 && (
                      <div className="space-y-5">
                        <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm flex flex-wrap gap-4 shadow-sm">
                          <div>
                            <span className="text-gray-500 text-xs block">Patient</span>
                            <span className="font-semibold text-gray-900">{newAppointmentData.name}</span>
                          </div>
                          <div className="w-px h-8 bg-blue-200"></div>
                          <div>
                            <span className="text-gray-500 text-xs block">Doctor</span>
                          ...
                          <div>
                            ...
                            <span className="font-semibold text-gray-900">{newAppointmentData.appointmentDate} at {newAppointmentData.appointmentTime}</span>
                          </div>
                        </div>
                          ...
                          <div className="grid gap-3 grid-cols-3">
                            {['Cash', 'UPI', 'Card'].map((method) => (
                              <button 
                                ...
                                type="button" 
                                onClick={() => { 
                                  setNewAppointmentData({ ...newAppointmentData, paymentMethod: method, upiId: '' }); 
                                  setErrorMsg(''); 
                                }} 
                            ...
                            <div className="mt-3 animate-in fade-in">
                              <label className="space-y-1 block">
                                ...
                                <input 
                                  ...
                                  onChange={(e) => setNewAppointmentData({ ...newAppointmentData, upiId: e.target.value })} 
                      ...
                      <div className="py-2">
                        <div className="max-w-md mx-auto bg-white border border-gray-200 rounded-xl overflow-hidden shadow mb-4">
                          <div className="bg-green-50 border-b border-green-100 px-5 py-4 flex items-center justify-between">
                            <div>
                              <p className="text-xs text-green-700 font-bold uppercase">Payment Successful</p>
                          ...
                          <div className="p-5 space-y-2.5 text-sm">
                            <div className="flex justify-between pb-2 border-b border-gray-100">
                              ...
                              <span className="font-bold text-gray-900 text-base">#{newAppointmentData.tokenNumber}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-500">Patient</span>
                              <span className="font-medium text-gray-900">{newAppointmentData.name}</span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="text-gray-900 font-medium">{newAppointmentData.appointmentDate} | {newAppointmentData.appointmentTime}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-500">Payment Date</span>
                              <span className="text-gray-900 font-medium">{newAppointmentData.paymentTimestamp}</span>
                            </div>
                            ...
                            <div className="border-t border-gray-100 pt-3 mt-2 flex justify-between text-base font-bold text-green-700 bg-green-50 p-2 rounded-lg -mx-2">
                              <span>Paid via {newAppointmentData.paymentMethod}</span>
              ...
              <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-md flex flex-col">
                  <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                    ...
                    <button 
                      onClick={() => { setDischargeModal(null); setErrorMsg(''); }} 
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                {['Cash', 'UPI', 'Card'].map(m => (
                                  <button 
                                    key={m} 
                                    onClick={() => { 
                                      setDischargeModal({ ...dischargeModal, method: m, upiId: '' }); 
                                      setErrorMsg(''); 
                                    }} 
                            ...
                            {dischargeModal.method === 'UPI' && (
                              <label className="block mt-2">
                                ...
                                <input 
                                  ...
                                  onChange={e => setDischargeModal({ ...dischargeModal, upiId: e.target.value })} 
